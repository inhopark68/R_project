...
# 반드시 non-responder/stable 을 먼저 판별해야 함
# -----------------------------
meta$group <- ifelse(
  grepl("control", meta$title, ignore.case = TRUE),
  "Control",
  ifelse(
    grepl("non-responder|stable", meta$title, ignore.case = TRUE),
    "Non-responder",
    ifelse(
...
meta$group <- factor(meta$group, levels = c("Control", "Non-responder", "Responder"))

cat("group table:\n")
print(table(meta$group, useNA = "ifany"))

if (any(is.na(meta$group))) {
  warning("Some samples could not be assigned to a group. Check meta$title.")
}
