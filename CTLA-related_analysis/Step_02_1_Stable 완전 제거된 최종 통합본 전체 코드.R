...
# Final integrated version (Stable removed)
# - groups: Control / Non-responder / Responder
# - Stable and Unclassified excluded
# - DESeq2 analysis
# - volcano plots with immune / melanogenesis gene labels
# - black gene labels in volcano plots
# =========================================================
...
# Stable is explicitly recognized but removed downstream
# -----------------------------
...
if (any(meta$group %in% c("Stable", "Unclassified"))) {
  cat("\nExcluded sample titles (Stable or Unclassified):\n")
  print(unique(meta$title[meta$group %in% c("Stable", "Unclassified")]))
}

# Remove Stable + Unclassified
keep_idx <- !meta$group %in% c("Stable", "Unclassified") & !is.na(meta$group)
...
cat("group table after removing Stable:\n")
print(table(meta$group, useNA = "ifany"))
cat("samples retained after filtering:", ncol(expr), "\n")
...
concept_long$group <- factor(concept_long$group, levels = c("Control", "Non-responder", "Responder"))
